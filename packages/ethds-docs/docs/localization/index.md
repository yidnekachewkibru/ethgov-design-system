...
- **Interdependent with [accessibility](/docs/accessibility):** correct
  `lang`, translated accessible names, and translated error messages are
  required for screen readers — see
  [screen-readers.md](https://github.com/yidnekachewkibru/ethgov-design-system/blob/main/docs/accessibility/screen-readers.md).
- **Implemented in components:** the i18n infrastructure and the
  [`LanguageSwitcher`](language-switcher.md) ship in
  [`@ethds/react`](/docs/components/language-switcher).
- **The docs site** already demonstrates the configured-locale model — the
  language dropdown at the top of this page is real, not a mock.
