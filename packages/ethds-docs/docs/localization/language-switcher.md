...
title: Language Switcher Standard
sidebar_position: 8
---
...
component ships as [`LanguageSwitcher`](/docs/components/language-switcher)
in `@ethds/react`; this page is its behavioural specification.

## Placement & presence

- Present on **every page**, in a consistent location — in the
  [Header](/docs/components/header), top-right in LTR layouts.
...
2. **The choice persists** across the entire journey and across visits:
   ...
   serves [Multilingual by Default](https://github.com/yidnekachewkibru/ethgov-design-system/blob/main/docs/design-principles/02-multilingual-by-default.md).
3. **Updates `lang`** on the document to the new locale (and `dir` if ever
   relevant), so screen readers switch pronunciation — see
   [screen-readers](https://github.com/yidnekachewkibru/ethgov-design-system/blob/main/docs/accessibility/screen-readers.md).
...
- Fully **keyboard operable** and screen-reader labelled, per the
  [keyboard](https://github.com/yidnekachewkibru/ethgov-design-system/blob/main/docs/accessibility/keyboard-navigation.md)
  and
  [screen-reader](https://github.com/yidnekachewkibru/ethgov-design-system/blob/main/docs/accessibility/screen-readers.md)
  standards.
