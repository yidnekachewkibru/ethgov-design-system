...
(`@ethds/react`) so every service — especially the
[Payment Flow](/docs/patterns/payment-flow) and
[Receipt Flow](/docs/patterns/receipt-flow) patterns — displays money
identically. `@ethds/patterns` ships exactly this as `formatBirr`.
...
- Be explicit on payment and receipt screens: show the currency code,
  ...
  [security/trust](https://github.com/yidnekachewkibru/ethgov-design-system/blob/main/docs/design-principles/08-security-and-privacy-by-design.md)
