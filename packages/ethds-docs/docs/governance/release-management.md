...
(`access: "restricted"`, since no package has actually been published to
npm yet), `.github/workflows/changeset-check.yml` fails a PR that touches
a publishable package without a changeset, and
...
4. The
   [documentation site](https://github.com/yidnekachewkibru/ethgov-design-system/tree/main/packages/ethds-docs)
   redeploys from `main` (GitHub Pages) automatically.
...
- Security fixes ship as **patch versions out of band**, coordinated per
  ...
  — they do not wait for the normal cadence.
