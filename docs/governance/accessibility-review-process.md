...
[Accessibility Framework](../accessibility/) as a repeatable review step,
so **WCAG 2.2 AA is enforced, not just declared**.
...
Against the [accessibility checklist](../accessibility/checklist.md):
...
- A change that violates a
  [policy non-negotiable](../accessibility/accessibility-policy.md#non-negotiables)
  is **blocked**, not merged with a "known issue".

## Continuous enforcement

Automated a11y checks run in CI on every PR touching UI packages (wired in
as those packages gain the test setup). Reported accessibility defects are
treated as bugs and prioritized. The
[testing framework](../accessibility/testing-framework.md) describes the
tooling.
