...
- **Atomic cross-package changes.** Early in the project, tokens,
  components, patterns, and templates change together constantly. A
  monorepo lets a single PR update `@ethds/tokens` and the
  `@ethds/react` components that consume it in one atomic, reviewable
  change — no publish-then-bump cycle between repos.
