...
[Localization Framework](../localization/) so "multilingual by default" is
enforced in review, and translations stay accurate and consistent.
...
- [ ] No hardcoded user-facing strings; all text externalized as
      translatable messages ([translation structure](../localization/translation-structure.md)).
...
- [ ] `lang` set correctly; the
      [Language Switcher](../localization/language-switcher.md) behaviour
      preserved.
...
- Missing translations fall back to English at the message level, so a
  ...
  close**, tracked as a [success metric](../foundation/success-metrics.md),
  not a silent omission.

The full workflow (author English → detect gaps → translate → review →
ship) is in the
[translation workflow](../localization/translation-workflow.md).
